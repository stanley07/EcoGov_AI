...
export function validateWorkflowGraph(
  steps: readonly WorkflowStepValidation[],
  transitions: readonly WorkflowTransitionValidation[],
): void {
  // 1. Enforce exactly one entry step
  const entrySteps = steps.filter((s) => s.isEntryStep);
  if (entrySteps.length !== 1) {
    throw new Error(
      `Workflow definition must have exactly one entry step. Found: ${entrySteps.length}`,
    );
  }
  ...
  if (terminalSteps.length === 0) {
    throw new Error(
      "Workflow definition must have at least one terminal step.",
    );
  }
  ...
  for (const trans of transitions) {
    if (trans.fromStep && terminalNames.has(trans.fromStep)) {
      throw new Error(
        `Workflow transition leaving terminal step is prohibited: ${trans.fromStep} -> ${trans.toStep}`,
      );
    }
  }
  ...
  for (const step of steps) {
    if (!step.isTerminalStep) {
      ...
      if (!reachesTerminal) {
        throw new Error(
          `Dead-end step detected: step "${step.stepName}" cannot reach any terminal step.`,
        );
      }
    }
  }
...
export async function createWorkflowInstance(
  ...
  entityType: string,
  entityId: string,
): Promise<{ instanceId: string; initialStepExecutionId: string }> {
  ...
  if (versionRes.rows.length === 0) {
    throw new Error(
      `Active workflow version and entry step not found for name: ${workflowName}`,
    );
  }
  ...
  const instanceRes = await client.query(
    `INSERT INTO workflow_instance (tenant_id, version_id, entity_type, entity_id, status)
     VALUES ($1, $2, $3, $4, 'active')
     RETURNING id`,
    [tenantId, version_id, entityType, entityId],
  );
  ...
  // 4. Log audit event
  await client.query(
    `INSERT INTO workflow_audit (tenant_id, workflow_instance_id, action, details)
     VALUES ($1, $2, 'initialize', $3)`,
    [tenantId, instanceId, JSON.stringify({ entityType, entityId, step_name })],
  );
...
export async function transitionWorkflowInstance(
  ...
  if (execRes.rows.length === 0) {
    throw new Error(
      `Active pending step execution not found for id: ${currentStepExecutionId}`,
    );
  }
  ...
  if (transRes.rows.length === 0) {
    throw new Error(
      `No transition defined for step "${step_name}" under outcome "${outcomeCode}"`,
    );
  }
