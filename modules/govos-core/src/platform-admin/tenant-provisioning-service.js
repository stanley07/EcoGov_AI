...
export class TenantProvisioningService {
     ...
     */
    async provision(actorUserId, idempotencyKey, rawInput) {
        // 1. Validate inputs
      ...
      `;
            ...
            if (idempRes.rows.length > 0) {
                ...
                if (record.status === "processing") {
                    ...
                    if (!tenantId) {
                        ...
                        const correlationCheck = await client.query(`SELECT tenant_id FROM authz_audit_log
               WHERE user_id = $1 AND action = 'TENANT_CREATED' AND context->>'idempotencyRecordId' = $2
               LIMIT 1`, [actorUserId, record.id]);
                    ...
                    if (!tenantId) {
                        ...
                        const fallbackCheck = await client.query(`SELECT tenant_id FROM authz_audit_log
               WHERE user_id = $1 AND action = 'TENANT_CREATED' AND context->>'slug' = $2
               LIMIT 1`, [actorUserId, input.slug]);
               ...
               WHERE id = $2`, [JSON.stringify(responsePayload), record.id, tenantId]);
                        await client.query("COMMIT");
                        return { status: 201, payload: responsePayload };
                    }
                    // Resource does not exist, claim a new lease
                    const leaseClaimRes = await client.query(`UPDATE idempotency_record
             SET locked_at = NOW(),
                 lock_owner = $1,
             ...
             WHERE id = $2
               ...
               AND (
                 ...
                 OR locked_at < NOW() - make_interval(secs => $3)
                    ...
                    const failedReclaimRes = await client.query(`UPDATE idempotency_record
             SET status = 'processing',
                 locked_at = NOW(),
                 lock_owner = $1,
                 attempt_count = attempt_count + 1,
                 updated_at = NOW()
             WHERE id = $2 AND status = 'failed'`, [lockOwner, record.id]);
                ...
                const insertRes = await client.query(`INSERT INTO idempotency_record (
           ...
           RETURNING id`, [actorUserId, idempotencyKey, operationName, requestHash, expiresAt, lockOwner]);
        ...
        try {
            ...
            const slugCheck = await txClient.query("SELECT id FROM tenant WHERE slug = $1", [input.slug]);
            if (slugCheck.rows.length > 0) {
                throw new Error(`CONFL:Slug ${input.slug} is already taken.`);
            }
            // Create Tenant
            const tenantId = crypto.randomUUID();
            await txClient.query(`INSERT INTO tenant (id, name, slug, type, status, is_system)
         VALUES ($1, $2, $3, $4, 'active', FALSE)`, [tenantId, input.name, input.slug, input.type]);
            // Create Organization
            const orgId = crypto.randomUUID();
            await txClient.query(`INSERT INTO organization (id, tenant_id, name, status)
         VALUES ($1, $2, $3, 'active')`, [orgId, tenantId, `${input.name} headquarters`]);
            // Resolve or create identity user
            let userId = "";
            const emailNormalized = input.adminEmail.trim().toLowerCase();
            const userCheck = await txClient.query("SELECT id, status FROM user_account WHERE tenant_id = $1 AND email = $2", [tenantId, emailNormalized]);
            if (userCheck.rows.length > 0) {
                ...
                await txClient.query(`INSERT INTO user_account (id, tenant_id, email, password_hash, first_name, last_name, status)
           VALUES ($1, $2, $3, $4, $5, '', 'invited')`, [userId, tenantId, emailNormalized, dummyHash, input.adminName]);
            }
            // Seed roles
            const roleMap = new Map();
            for (const r of DEFAULT_SEED_ROLES) {
                const rId = crypto.randomUUID();
                await txClient.query(`INSERT INTO role (id, tenant_id, name, description, is_system)
           VALUES ($1, $2, $3, $4, TRUE)`, [rId, tenantId, r.name, r.description]);
            ...
            for (const perm of DEFAULT_SEED_PERMISSIONS) {
                const permRes = await txClient.query(`INSERT INTO permission (tenant_id, name, description)
           VALUES ($1, $2, $3) RETURNING id`, [tenantId, perm, `Permission to ${perm}`]);
                const permId = permRes.rows[0].id;
                // Map everything to super_admin
                await txClient.query(`INSERT INTO role_permission (role_id, permission_id) VALUES ($1, $2)`, [superAdminRoleId, permId]);
                // Map officer perms
                if (OFFICER_PERMS.includes(perm)) {
                    await txClient.query(`INSERT INTO role_permission (role_id, permission_id) VALUES ($1, $2)`, [directorRoleId, permId]);
                    await txClient.query(`INSERT INTO role_permission (role_id, permission_id) VALUES ($1, $2)`, [inspectorRoleId, permId]);
                }
            }
            // Create membership
            await txClient.query(`INSERT INTO membership (tenant_id, user_id, organization_id, role_id, status)
         VALUES ($1, $2, $3, $4, 'invited')`, [tenantId, userId, orgId, superAdminRoleId]);
            // Create hashed invitation
            const rawToken = crypto.randomBytes(32).toString("base64url");
            const tokenHash = crypto.createHash("sha256").update(rawToken).digest("hex");
            const invitationId = crypto.randomUUID();
            const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
            await txClient.query(`INSERT INTO user_invitation (
          ...
          role_id, token_hash, status, expires_at, created_by, updated_at
            ...
            await txClient.query(`INSERT INTO task_execution (
           tenant_id, task_id, task_type, payload_hash, status, available_at, attempt_count, max_attempts, encrypted_payload
         ) VALUES ($1, $2, 'govos.notification.invitation.send', $3, 'pending', NOW(), 0, 5, $4)`, [tenantId, taskId, tokenHash, JSON.stringify(encryptedEnvelope)]);
            // Audit Log with explicit correlation metadata
            await txClient.query(`INSERT INTO authz_audit_log (tenant_id, user_id, action, resource, result, context)
         VALUES ($1, $2, 'TENANT_CREATED', $3, 'allow', $4)`, [
            ...
            const idempUpdateRes = await txClient.query(`UPDATE idempotency_record
         ...
         WHERE id = $2 AND lock_owner = $3 AND status = 'processing'`, [JSON.stringify(responsePayload), recordId, lockOwner, tenantId]);
            ...
            if (err.message && err.message.startsWith("CONFL:")) {
                ...
                await this.pool.query(`UPDATE idempotency_record
           ...
           WHERE id = $2 AND lock_owner = $3`, [JSON.stringify({ error: msg }), recordId, lockOwner]);
         ...
         WHERE id = $2 AND lock_owner = $3`, [JSON.stringify({ error: err.message }), recordId, lockOwner]);
            throw err;
        }
        finally {
            txClient.release();
