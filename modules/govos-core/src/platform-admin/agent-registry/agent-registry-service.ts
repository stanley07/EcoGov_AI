...
export const PLATFORM_LIMIT_MAXS: AgentLimits = {
  timeout_seconds: 300,
  max_model_turns: 20,
  max_tool_calls: 10,
...
export class AgentRegistryService {
  ...
  private validateJsonSchema(schema: unknown): void {
    try {
  ...
  private validateLimits(limits: AgentLimits): void {
    if (
  ...
  public async createApplication(key: string, displayName: string): Promise<string> {
    const keyRegex = /^[a-z0-9-]+$/;
    if (!keyRegex.test(key)) {
      throw new Error("Application key must be lowercase alphanumeric with hyphens");
    }
    const res = await this.pool.query(
      `INSERT INTO application (key, display_name) 
       VALUES ($1, $2) 
       RETURNING id`,
      [key, displayName]
    );
  ...
  public async createAgentDefinition(key: string, displayName: string, owningApplicationId: string): Promise<string> {
    ...
    const res = await this.pool.query(
      `INSERT INTO agent_definition (key, display_name, owning_application_id, status) 
       VALUES ($1, $2, $3, 'active') 
       RETURNING id`,
      [key, displayName, owningApplicationId]
    );
    return res.rows[0].id;
  }

  // 3. Prompts draft management
  public async createPromptDefinition(key: string, owningApplicationId: string): Promise<string> {
    const appRes = await this.pool.query(`SELECT id FROM application WHERE id = $1`, [owningApplicationId]);
    if (appRes.rows.length === 0) {
      throw new RegistryError(RegistryErrorCode.APPLICATION_NOT_FOUND, `Application ${owningApplicationId} not found`);
    }
    ...
    const res = await this.pool.query(
      `INSERT INTO prompt_definition (key, owning_application_id) 
       VALUES ($1, $2) 
       RETURNING id`,
      [key, owningApplicationId]
    );
    return res.rows[0].id;
  }
  ...
  public async createPromptVersion(
    ...
    const res = await this.pool.query(
      `INSERT INTO prompt_version (prompt_definition_id, version, template, variables_schema, content_hash, status) 
       VALUES ($1, $2, $3, $4, $5, 'draft') 
       RETURNING id`,
      [promptDefinitionId, version, template, JSON.stringify(variablesSchema), contentHash]
    );
    return res.rows[0].id;
  }

  // 4. Output Contracts draft management
  public async createOutputContractDefinition(key: string, owningApplicationId: string): Promise<string> {
    const appRes = await this.pool.query(`SELECT id FROM application WHERE id = $1`, [owningApplicationId]);
    if (appRes.rows.length === 0) {
      throw new RegistryError(RegistryErrorCode.APPLICATION_NOT_FOUND, `Application ${owningApplicationId} not found`);
    }
    ...
    const res = await this.pool.query(
      `INSERT INTO output_contract_definition (key, owning_application_id) 
       VALUES ($1, $2) 
       RETURNING id`,
      [key, owningApplicationId]
    );
  ...
  public async createOutputContractVersion(
    outputContractDefinitionId: string,
    version: string,
    jsonSchema: unknown
  ): Promise<string> {
    ...
    const res = await this.pool.query(
      `INSERT INTO output_contract_version (output_contract_definition_id, version, json_schema, content_hash, status) 
       VALUES ($1, $2, $3, $4, 'draft') 
       RETURNING id`,
      [outputContractDefinitionId, version, JSON.stringify(jsonSchema), contentHash]
    );
    return res.rows[0].id;
  }

  // 5. Agent Versions draft management
  public async createAgentVersion(
  ...
  public async createToolDefinition(key: string, category: string): Promise<string> {
    const res = await this.pool.query(
      `INSERT INTO tool_definition (key, category) 
       VALUES ($1, $2) 
       RETURNING id`,
      [key, category]
    );
  ...
  public async createToolVersion(
    ...
    if (outputSchema) this.validateJsonSchema(outputSchema);
  ...
  public async linkAgentVersionTool(agentVersionId: string, toolVersionId: string): Promise<void> {
    await this.pool.query(
      `INSERT INTO agent_version_tool (agent_version_id, tool_version_id) 
       VALUES ($1, $2) 
       ON CONFLICT DO NOTHING`,
      [agentVersionId, toolVersionId]
    );
  }

  // 7. Transactional Activation
  public async activateAgentVersion(agentVersionId: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");

      // A. Lock and retrieve Agent Version
      const agentVerRes = await client.query(
        `SELECT * FROM agent_version WHERE id = $1 FOR UPDATE`,
        [agentVersionId]
      );
      ...
      const agentDefRes = await client.query(
      ...
      const contractVerRes = await client.query(
        `SELECT ocv.*, ocd.owning_application_id 
         ...
         WHERE ocv.id = $1`,
        [agentVersion.output_contract_version_id]
      );
      ...
      if (
        agentDef.owning_application_id !== promptVersion.owning_application_id ||
  ...
  public async activatePromptVersion(promptVersionId: string): Promise<void> {
    const res = await this.pool.query(
  ...
  public async activateOutputContractVersion(contractVersionId: string): Promise<void> {
    const res = await this.pool.query(
      `UPDATE output_contract_version 
       SET status = 'active' 
       WHERE id = $1 AND status = 'draft' 
       RETURNING id`,
      [contractVersionId]
  ...
  public async activateToolVersion(toolVersionId: string): Promise<void> {
    const res = await this.pool.query(
      `UPDATE tool_version 
       SET status = 'active' 
       WHERE id = $1 AND status = 'draft' 
  ...
  public async retireAgentVersion(agentVersionId: string): Promise<void> {
    const res = await this.pool.query(
      `UPDATE agent_version 
  ...
  public async resolveActiveAgentVersion(agentDefinitionId: string): Promise<any> {
    const res = await this.pool.query(
      `SELECT * FROM agent_version 
       WHERE agent_definition_id = $1 AND status = 'active'`,
      [agentDefinitionId]
    );
    if (res.rows.length === 0) {
  ...
  public async resolveHistoricalAgentVersion(agentVersionId: string): Promise<any> {
    const res = await this.pool.query(
      `SELECT * FROM agent_version WHERE id = $1`,
      [agentVersionId]
    );
    if (res.rows.length === 0) {
      throw new RegistryError(RegistryErrorCode.AGENT_DEFINITION_NOT_FOUND, "Agent version not found");
